...
if __name__ == "__main__":
    tools_config_file = "/root/pbrs-rlvr/sandbox_fusion_tool_config.yaml"
    tools_config = OmegaConf.load(tools_config_file)
    tool_schema = OmegaConf.to_container(tools_config["tools"][0]["tool_schema"])
    tools = json.dumps([tool_schema])

    local_dataset_path = "/root/autodl-tmp/datasets/ReTool-SFT"
    data = datasets.load_dataset(local_dataset_path)["train"]
    data = data.map(process, fn_kwargs={"tools": tools})
    save_path = os.path.expanduser("/root/autodl-tmp/datasets/ReTool-SFT/data/train-00000-of-00001.parquet")
