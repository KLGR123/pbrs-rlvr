...
def _inject_igpo_ig_rewards(
    ...
    """Post-rollout IG reward computation for IGPO.

    Parses turn boundaries from sglang multi-turn responses, reconstructs
    per-turn contexts, calls compute_log_prob for GT log probs, computes
    IG as consecutive differences, writes ig_rewards to
    batch.non_tensor_batch["ig_rewards"], then builds the full per-token
    rm_scores tensor via IGPORewardManager and writes it to
    batch.batch["rm_scores"] in-place.

    This function must run before the reward block in the training loop so
    that the reward loop workers see rm_scores already present and skip
    their own (scalar-only) computation.

    When ig_config.ig_source == "ref", the frozen reference model is used
    for GT log-prob computation instead of the live policy.  ref_policy_wg
    ...
    valid_fields = IGRewardConfig.__dataclass_fields__
    config = IGRewardConfig(**{k: v for k, v in ig_config_dict.items() if k in valid_fields})
    computer = IGRewardComputer(tokenizer, config)

    bsz = batch.batch["responses"].shape[0]
    prompt_len = batch.batch["prompts"].shape[1]
    attn = batch.batch["attention_mask"]

    # ── Ground truths ──────────────────────────────────────────────────
    reward_model_data = batch.non_tensor_batch.get("reward_model", [{}] * bsz)
    gt_token_ids_list, gt_token_ranges = [], []
    for i in range(bsz):
        rm = reward_model_data[i]
        gt = rm.get("ground_truth", "") if isinstance(rm, dict) else str(rm)
        gt_ids, gt_range = computer.prepare_gt_tokens(gt)
        gt_token_ids_list.append(gt_ids)
        gt_token_ranges.append(gt_range)

    # ── Parse turn boundaries from response strings ────────────────────
    # Use skip_special_tokens=False so that turn_separator special tokens
    # are preserved for accurate boundary detection.
    all_resp_strs, all_sep_positions = [], []
    for i in range(bsz):
        valid_resp_len = int(attn[i, prompt_len:].sum().item())
        resp_ids = batch.batch["responses"][i, :valid_resp_len].tolist()
        resp_str = tokenizer.decode(resp_ids, skip_special_tokens=False)
        all_resp_strs.append(resp_str)

        positions, pos = [], 0
        while True:
            idx = resp_str.find(turn_separator, pos)
            if idx == -1:
                break
            positions.append(idx)
            pos = idx + 1
        all_sep_positions.append(positions)

    max_turns = max((len(s) + 1 for s in all_sep_positions), default=1)
    if max_turns <= 1:
        # Single-turn: no IG rewards needed; outcome reward only.
        # Must be a numpy object array so that batch.reorder() can index it.
        batch.non_tensor_batch["ig_rewards"] = np.array(
            [[] for _ in range(bsz)], dtype=object
        )
    else:
        # ── Build turn_context_snapshots[t][i] ────────────────────────
        # turn_context_snapshots[t][i]: context dict at turn t for sample i.
        # t=0: prompt only (before any response); t>0: prompt + response up
        # to the (t-1)-th separator character boundary.
        # Use the prompts tensor directly — it is already right-aligned
        # (valid tokens at the end), matching attention_mask.
        turn_context_snapshots = []
        for t in range(max_turns):
            turn_contexts = []
            for i in range(bsz):
                sep_positions = all_sep_positions[i]
                valid_prompt_len = int(attn[i, :prompt_len].sum().item())
                # prompts tensor is right-aligned: valid tokens are the last
                # valid_prompt_len positions.
                p_ids = batch.batch["prompts"][i, prompt_len - valid_prompt_len :]
                p_mask = torch.ones(valid_prompt_len, dtype=torch.long)
                p_pos  = torch.arange(valid_prompt_len, dtype=torch.long)

                if t == 0:
                    # Before first response: context = prompt only
                    turn_contexts.append({
                        "input_ids": p_ids,
                        "attention_mask": p_mask,
                        "position_ids": p_pos,
                    })
                elif t - 1 < len(sep_positions):
                    # Context = prompt + response prefix up to separator t-1.
                    # Re-tokenize the decoded prefix to get exact token IDs.
                    resp_prefix = all_resp_strs[i][: sep_positions[t - 1]]
                    r_ids = tokenizer(
                        resp_prefix, add_special_tokens=False, return_tensors="pt"
                    )["input_ids"].squeeze(0)
                    ctx_ids  = torch.cat([p_ids, r_ids])
                    ctx_mask = torch.ones(len(ctx_ids), dtype=torch.long)
                    ctx_pos  = torch.arange(len(ctx_ids), dtype=torch.long)
                    turn_contexts.append({
                        "input_ids": ctx_ids,
                    ...
                    turn_contexts.append(None)  # sample already finished
            turn_context_snapshots.append(turn_contexts)

        # ── Select worker group and method based on ig_source ─────────
        if config.ig_source == "ref":
            assert ref_policy_wg is not None, (
                "ig_config.ig_source='ref' requires ref_policy_wg to be passed "
                "to _inject_igpo_ig_rewards, but it is None. "
                "Ensure actor_rollout_ref.actor.use_reference_policy=True."
            )
            _wg = ref_policy_wg
            _method = "compute_ref_log_prob"
            _key = "ref_log_prob"
        else:
            _wg = actor_rollout_wg
            _method = "compute_log_prob"
            _key = "old_log_probs"

        # ── Batch compute IG rewards (one compute_log_prob call) ───────
        ig_rewards = computer.compute_all_ig_batched(
            turn_context_snapshots=turn_context_snapshots,
            gt_token_ids_list=gt_token_ids_list,
            gt_token_ranges=gt_token_ranges,
            actor_rollout_wg=_wg,
            log_prob_method=_method,
            log_prob_key=_key,
        )
        # Must be a numpy object array so that batch.reorder() can index it.
        batch.non_tensor_batch["ig_rewards"] = np.array(ig_rewards, dtype=object)

    # ── Build per-token rm_scores via IGPORewardManager ───────────────
    # This must happen after ig_rewards is written so that the manager can
    # place them at the correct turn-end token positions.  Writing rm_scores
    # here causes the subsequent reward block to skip _compute_reward_colocate
    # (which would overwrite with a last-token-only scalar).
    reward_mgr = IGPORewardManager(tokenizer=tokenizer, turn_separator=turn_separator)
    batch.batch["rm_scores"] = reward_mgr(batch, return_dict=False)
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.IGPO:
                        with marked_timer("igpo_ig_rewards", timing_raw, color="cyan"):
                            _inject_igpo_ig_rewards(
                                batch=batch,
                                tokenizer=self.tokenizer,
                                actor_rollout_wg=self.actor_rollout_wg,
                                ig_config_dict=dict(self.config.algorithm.get("ig_config", {})),
                                turn_separator=self.config.algorithm.get(
                                    "turn_separator", "\n<|im_start|>assistant\n"
                                ),
                                ref_policy_wg=getattr(self, "ref_policy_wg", None),
                            )
