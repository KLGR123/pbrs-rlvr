...
# ---------------------------------------------------------------------------
# IGPO: Information Gain-based Policy Optimization advantage estimator
# ---------------------------------------------------------------------------

def _compute_igpo_turn_level_returns(
    normalized_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    turn_boundary_mask: torch.Tensor,
    gamma: float,
) -> torch.Tensor:
    """Turn-level discounted accumulation + broadcast for IGPO.

    For each sample, identifies turn boundaries (positions marked in
    turn_boundary_mask), performs backward discounted accumulation
    R_t = r_t + γ * R_{t+1}, then broadcasts R_t to every token that
    belongs to turn t.

    Args:
        normalized_rewards: (bsz, seq_len) normalised reward tensor.
        response_mask: (bsz, seq_len) binary mask for valid response tokens.
        turn_boundary_mask: (bsz, seq_len) binary mask marking the last
            token of each turn (where a reward was originally placed).
        gamma: Discount factor.

    Returns:
        discounted_returns: (bsz, seq_len), same dtype as normalized_rewards.
    """
    bsz, seq_len = normalized_rewards.shape
    device = normalized_rewards.device
    discounted_returns = torch.zeros(bsz, seq_len, device=device,
                                     dtype=normalized_rewards.dtype)

    for b in range(bsz):
        reward_positions = (
            turn_boundary_mask[b].nonzero(as_tuple=True)[0].tolist()
        )
        if not reward_positions:
            continue

        # Backward discounted accumulation over turns
        turn_data: list[tuple[int, float]] = []
        next_adv = 0.0
        for pos in reversed(reward_positions):
            r = normalized_rewards[b, pos].item()
            adv = r + gamma * next_adv
            turn_data.append((pos, adv))
            next_adv = adv
        turn_data.reverse()  # Forward order

        # Broadcast each turn's advantage to its token span
        prev_end = 0
        for reward_pos, adv in turn_data:
            for t in range(prev_end, reward_pos + 1):
                if response_mask[b, t] == 1:
                    discounted_returns[b, t] = adv
...
def compute_igpo_advantage(
    ...
    **kwargs,
    ...
    """IGPO advantage estimation with turn-level discounted returns.

    Implements the full IGPO reward-processing pipeline described in the
    paper (Section 3.3):

    1. Separate z-normalisation of IG rewards and outcome rewards within
       each prompt group (or joint normalisation if configured).
    2. Optional curriculum weighting of IG vs. outcome rewards.
    3. Turn-level discounted return: R_t = r̃_t + γ * R_{t+1}.
    4. Broadcast R_t to every token in turn t.

    The reward tensor is expected to have been filled by IGPORewardManager:
      - IG reward (or sentinel 1e-10) at the last token of each
        intermediate turn.
      - Outcome reward at the last valid token of the response.

    Config keys read (all optional, with defaults):
      ig_norm_mode (str): "separate" (default) or "joint".
          "separate" normalises IG and outcome rewards independently.
          "joint" normalises all reward positions together.
      ig_reward_weight (float): Multiplicative weight for IG rewards
          before normalisation (default 1.0).
      outcome_reward_weight (float): Multiplicative weight for outcome
          rewards before normalisation (default 1.0).
      gamma (float): Turn-level discount factor (default 1.0).

    Args:
        token_level_rewards: (bsz, resp_len) token-level reward tensor
            produced by IGPORewardManager.
        response_mask: (bsz, resp_len) binary mask for valid tokens.
        index: (bsz,) group IDs (uids from non_tensor_batch) for
            group-wise z-normalisation.
        epsilon: Numerical stability constant.
        norm_adv_by_std_in_grpo: Whether to divide by group std.
        config: AlgoConfig (or dict-like) containing IGPO settings.

    Returns:
        advantages: (bsz, resp_len) — turn-level discounted returns
            broadcast to token level.
        returns: same tensor (IGPO uses advantages == returns).
    """
    # Read IGPO-specific config
    cfg = config or {}
    ig_norm_mode: str = cfg.get("ig_norm_mode", "separate")
    ig_reward_weight: float = float(cfg.get("ig_reward_weight", 1.0))
    outcome_reward_weight: float = float(cfg.get("outcome_reward_weight", 1.0))
    gamma: float = float(cfg.get("gamma", 1.0))
    ...
    with torch.no_grad():
        # ------------------------------------------------------------------
        # Step 1: Identify reward positions
        # ------------------------------------------------------------------
        # outcome_mask: last valid token per sample (carries outcome reward)
        last_valid_pos = (
            (seq_len - 1)
            - response_mask.flip(dims=[1]).to(torch.long).argmax(dim=1)
        )
        pos_indices = torch.arange(seq_len, device=device).unsqueeze(0).expand(bsz, -1)
        outcome_mask = (
            (pos_indices == last_valid_pos.unsqueeze(1)) & (response_mask == 1)
        )
        # ig_mask: non-zero reward positions that are NOT the outcome position
        ig_mask = (
            (response_mask == 1)
            & (~outcome_mask)
            & (token_level_rewards != 0)
        )
        turn_boundary_mask = outcome_mask | ig_mask

        # ------------------------------------------------------------------
        # Step 2: Apply curriculum weights
        # ------------------------------------------------------------------
        weighted_rewards = token_level_rewards.clone()
        if ig_reward_weight != 1.0:
            weighted_rewards = torch.where(
                ig_mask,
                weighted_rewards * ig_reward_weight,
                weighted_rewards,
            )
        if outcome_reward_weight != 1.0:
            weighted_rewards = torch.where(
                outcome_mask,
                weighted_rewards * outcome_reward_weight,
                weighted_rewards,
            )

        # ------------------------------------------------------------------
        # Step 3: Group mapping (convert arbitrary UIDs → contiguous IDs)
        # ------------------------------------------------------------------
        unique_indices, inverse = np.unique(index, return_inverse=True)
        group_ids = torch.tensor(inverse, device=device, dtype=torch.long)  # (bsz,)
        num_groups = len(unique_indices)
        group_ids_exp = group_ids.unsqueeze(1).expand(-1, seq_len)  # (bsz, seq_len)

        # ------------------------------------------------------------------
        # Step 4: Group-wise z-normalisation
        # ------------------------------------------------------------------
        def _group_normalise(mask: torch.Tensor) -> torch.Tensor:
            """Z-normalise weighted_rewards at mask positions within groups."""
            flat_mask = mask.view(-1)
            flat_rewards = weighted_rewards.view(-1)
            flat_gids = group_ids_exp.reshape(-1)

            valid_idx = flat_mask.nonzero(as_tuple=True)[0]
            if valid_idx.numel() == 0:
                return torch.zeros_like(weighted_rewards)

            vr = flat_rewards[valid_idx]
            vg = flat_gids[valid_idx]

            g_sum = torch.zeros(num_groups, device=device).scatter_add_(0, vg, vr)
            g_cnt = torch.zeros(num_groups, device=device).scatter_add_(
                0, vg, torch.ones_like(vr)
            )
            g_mean = g_sum / g_cnt.clamp(min=1.0)

            sq_diff = (vr - g_mean[vg]) ** 2
            g_sq = torch.zeros(num_groups, device=device).scatter_add_(0, vg, sq_diff)
            g_var = g_sq / g_cnt.clamp(min=1.0)
            g_std = torch.sqrt(g_var + 1e-8)
            g_std = torch.where(g_cnt <= 1, torch.ones_like(g_std), g_std)

            mean_map = g_mean[group_ids_exp]  # (bsz, seq_len)
            std_map = g_std[group_ids_exp]

            norm = weighted_rewards - mean_map
            if norm_adv_by_std_in_grpo:
                norm = norm / (std_map + epsilon)
            out = torch.zeros_like(weighted_rewards)
            out = torch.where(mask, norm, out)
            return out

        normalised = torch.zeros_like(weighted_rewards)

        if ig_norm_mode == "separate":
            # Normalise IG rewards and outcome rewards independently
            normalised = normalised + _group_normalise(ig_mask)
            normalised = normalised + _group_normalise(outcome_mask)
        else:
            # Joint normalisation over all reward positions
            normalised = _group_normalise(turn_boundary_mask)

        # ------------------------------------------------------------------
        # Step 5: Turn-level discounted return + broadcast
        # ------------------------------------------------------------------
        discounted_returns = _compute_igpo_turn_level_returns(
            normalized_rewards=normalised,
            response_mask=response_mask,
